...
export const defaultTranslators: TranslatorConfigObject = {
  ...
  'li': ({
    ...
    return {
      prefix:
        indent.repeat(+(indentLevel || 0)) +
        (listKind === 'OL' && listItemNumber !== undefined
          ? `${listItemNumber}. `
          : `${bulletMarker} `),
  ...
  'blockquote': {
    postprocess: ({ content }) =>
      trimNewLines(content).replace(/^(>*)[^\S\r\n]?/gm, `>$1 `),
  },
  ...
  'code': ({
    ...
    const isCodeBlock =
      ['PRE', 'WRAPPED-PRE'].includes((parent?.tagName || '').toUpperCase()) &&
  ...
  'table': ({ visitor }) => ({
    ...
    postprocess: ({ content, nodeMetadata, node }) => {
      ...
      const rawRows = splitSpecial(content).map(({ text }) =>
        text.replace(/^(?:\|)?(.+)\s*\|\s*$/, '$1'),
      );
      ...
      let res = '';
      const { caption } = nodeMetadata.get(node)!.tableMeta!;
...
export const tableTranslatorConfig: TranslatorConfigObject = {
  ...
  'tr': ({ visitor }) => ({
    ...
    prefix: '| ',
    postprocess: ({ content }) =>
      !/ \|\s*$/.test(content) ? PostProcessResult.RemoveNode : content,
  }),
