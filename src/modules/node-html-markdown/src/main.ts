...
import {
  aTagTranslatorConfig,
  defaultBlockElements,
  defaultCodeBlockTranslators,
  defaultIgnoreElements,
  defaultOptions,
  defaultTranslators,
  tableCellTranslatorConfig,
  tableRowTranslatorConfig,
...
export class NodeHtmlMarkdown {
  ...
  constructor(
    ...
    const ignoredElements =
      this.options.ignore?.concat(defaultIgnoreElements) ??
      defaultIgnoreElements;
    const blockElements =
      this.options.blockElements?.concat(defaultBlockElements) ??
      defaultBlockElements;
    ...
    // Strip doctypes that can surface as text in non-native parser output.
  ...
  static translate(
    htmlOrFiles: string | FileCollection,
    opt?: Options,
    customTranslators?: TranslatorConfigObject,
    customCodeBlockTranslators?: TranslatorConfigObject,
  ): string | FileCollection {
    return NodeHtmlMarkdown.prototype.translateWorker.call(
      new NodeHtmlMarkdown(opt, customTranslators, customCodeBlockTranslators),
      htmlOrFiles,
    );
  }
  ...
  private translateWorker(htmlOrFiles: string | FileCollection) {
    ...
    const inputFiles: FileCollection = !inputIsCollection
      ? { 'default': <string>htmlOrFiles }
      : <FileCollection>htmlOrFiles;
    ...
    for (const [fileName, html] of Object.entries(inputFiles)) {
      ...
      outputFiles[fileName] = getMarkdownForHtmlNodes(
        this,
        parsedHtml,
        fileName !== 'default' ? fileName : undefined,
      );
    }
