  ...
  private optimizeTree(node: HtmlNode) {
    ...
    (function visit(node: HtmlNode): boolean {
      ...
      if (
        isTextNode(node) ||
        (isElementNode(node) &&
          contentlessElements.includes((node.tagName || '').toUpperCase()))
      ) {
        res = true;
      } else {
        ...
        if (!childNodes.length) {
          ...
          const translator = elementNode.tagName
            ? translators.get(elementNode.tagName)
            : undefined;
          if (typeof translator === 'function' || translator?.preserveIfEmpty) {
            res = true;
          }
        } else {
  ...
  public visitNode(
    ...
    if (isTextNode(node)) {
      ...
      if (node.isWhitespace && !metadata?.preserveWhitespace) {
        return !result.text.length || result.trailingNewlineStats.whitespace > 0
          ? void 0
          : this.appendResult(' ');
      }

      // Fix for issues #61 and #34: Process original text to preserve trailing whitespace before inline elements
      const sourceText = metadata?.preserveWhitespace
        ? node.text
        : node.text || node.trimmedText;
    ...
    const translatorCfgOrFactory:
      | TranslatorConfig
      | TranslatorConfigFactory
      | undefined = tagNameUpper
      ? metadata?.translators
        ? metadata.translators[tagNameUpper]
        : this.instance.translators.get(node.tagName)
      : undefined;
    ...
    switch (tagNameUpper) {
      ...
      case 'OL':
        metadata = {
          ...metadata,
          listItemNumber: 0,
          listKind: tagNameUpper as 'OL' | 'UL',
    ...
    if (typeof cfg.content === 'string') {
      ...
      if (cfg.postprocess) {
        ...
        if (postRes === PostProcessResult.RemoveNode) {
          if (tagNameUpper === 'LI' && metadata?.listItemNumber) {
